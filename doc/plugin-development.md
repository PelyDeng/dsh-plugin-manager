...
插件放在 `plugins/*`，需要 `files`、README、`build` 和 `check`；`description` 可省略。构建由管理器按 build → check → pack 执行，不能用 prepare/prepack/postpack 重复构建。`files` 仅包含公开产物，用户配置通过 `runtimeConfig` 声明，公开模板可选。发现时允许尚未生成的入口，打包时必须提供全部声明文件。
