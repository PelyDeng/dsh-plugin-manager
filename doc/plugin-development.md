...
需要 Node.js `^22.19.0 || >=24`、pnpm `11.19.0` 和系统 `tar`。准备一个作者仓库之外的工具目录，例如 `dsh-tools`。取得维护者提供的 manager 0.3.2 tgz，核对提供方 SHA-256；需要统一身份时再取得 kit 0.1.1 tgz。包名不表示已公开发布到 npm。
...
```sh
pnpm add --ignore-workspace --save-dev "/absolute/path/plugin-kit-0.1.1.tgz"
```
