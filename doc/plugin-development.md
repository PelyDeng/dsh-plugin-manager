...
首次运行按[配置并启动](getting-started.md#3-配置并启动)操作。站点 build 仅支持 release；底层 CLI 的 development 与管理参数见[字段参考](framework-configuration.md#独立-cli-字段参考)。示例默认要求登录，候选选入 auth、example 并配置公开 origin。安装现成清单默认选择其中全部插件，也可通过 `--plugins` 限定。
