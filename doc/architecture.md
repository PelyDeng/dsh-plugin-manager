...
**插件目录是数据契约，不是某个插件的界面。** 谁在场、叫什么名字、属于哪个分类，由 kit 定义并传输：插件用 `registerPlugin()` 登记，任何一方用 `listPlugins()` 读取，中间走 `ecosystem/catalog` 事件；分类标签与固定顺序也在 kit 里，保证所有界面看到同一套名称。界面各自决定怎么展示这份契约 —— auth 只是其中一种渲染（按分类分区的目录页），别的消费方读取同一份契约、自己组织视图。因此新增界面不需要动契约，新增分类要改 kit 里的标签定义，而那些标签只用于展示，不参与授权判定。

## 源码、安装包和数据放在哪里

根 workspace 的源码插件发现只扫描 `plugins/builtin/*`；产物 build 只扫描站点 incoming 直接子目录的完整发布清单，两者由明确的 source/archives 模式选择。开发者在独立仓库开发单个包时，用 `--root <作者包根> --package .` 指定它的位置，仍使用相同的任务和归档校验。
...
普通 build 每次从当前现场重新收敛：受管授权集合（profile 状态 schema 3）承接部分失败，失败后修正输入直接重跑，不迁移目录、不自动回滚业务数据。停旧前会核验候选挂载与权限、停旧后核实旧容器的持久挂载对应站点绑定，但不会自动备份全部运行数据。数据备份与恢复由运维单独安排。

发布清单可选的 verification 记录构建输入、归档测试、最终 tgz 的摘要，以及测试过的插件组合。管理器用同一模块处理记录校验、合并、插件选择和安装提示；这些记录不参与安装授权集合的计算，也不改变 kit 或插件声明。测试记录与部署环境标识分开核对；信息不足、无法确认对应关系时，结果标为未知。协议与交付流程见[发布物验证记录](../packages/plugin-manager/VERIFICATION.md)。
