...
管理员打开 `/auth` →“模型设置”，选择模型卡片右上角的单选项，自动保存到官方宿主设置，无需重启。仅新会话使用新默认，已有会话和分支沿用官方会话记录中的模型。模型密钥在下方“服务商 API 密钥”中管理；选择模型不会验证余额或执行付费请求。第三方插件需要接入 kit 的 `conversationModel()`，硬编码模型不会自动改变；专用识图等能力路由可由插件保留。
