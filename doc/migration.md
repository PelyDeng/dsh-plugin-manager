...
应用时增加 `--apply --stopped-file <json>`。停写记录必须生成于最近 15 分钟内，注明源和目标，并确认所有写入进程已停止：数据包括 DSH、插件和数据库写入进程；产物包括构建、打包、推送与恢复任务。
...
复制成功后，显式更新部署配置和挂载路径，再验证原账号、历史、工作文件与插件配置。tgz、清单和历史操作记录保持原字节；旧记录中的绝对路径不批量替换。副本里的旧部署记录只是历史材料：恢复参数（`--resume`/`--recover`）已移除，副本上直接运行普通 build 重新收敛，不继承原操作的工具与包选择。

目标开始写入后，回退需要再次停写、保存新增数据并确认处理方式。不能直接切回旧副本丢弃新会话。原目录和备份只在维护者明确要求时清理；普通 clean 保留所有运行数据和恢复产物。
...
pnpm 报 `ERR_PNPM_UNEXPECTED_STORE` 时，环境变化不再需要额外开关确认（`--rebuild` 已移除）。先停止目标副本的宿主，使用官方 CLI 的 `plugin --profile <profile> install --offline --store-dir <目标 store> --cache-dir <目标 cache> --config.force=true` 重装安装目录；离线包中必须备齐所有直接和间接依赖，业务数据和原备份保持完整。随后按[新版迁移入口](first-deployment.md)用 `dsh-plugin-manager migrate-site` 处置旧受管状态与 `pending`：确认停写后导入授权，旧记录移入迁移备份。不能靠删除状态或删锁绕过迁移检查。
