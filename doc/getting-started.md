...
普通使用者从框架 **0.16.0** 的发行附件取得部署包和 public-apps.zip。public-apps 中的完整目录同时包含 auth/example；复制到 incoming/public-apps 后，不要再复制 optional/auth，以免重复 ID。
...
pnpm --filter @dsh-plugin-manager/plugin-manager pack --out .local/artifacts/plugin-manager-0.16.0.tgz
pnpm --filter @dsh-plugin-manager/plugin-kit pack --out .local/artifacts/plugin-kit-0.16.0.tgz
```
