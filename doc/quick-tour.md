...
用已授权的普通账号打开 `/example`，点击快捷问题或输入自己的问题，观察流式回答。通过顶栏“历史对话”打开历史弹层并继续会话。

思考默认折叠并显示单行预览，点击可展开；生成时可停止，已生成内容保留。回答下方提供复制、赞/踩、分支和真实用量、耗时详情。手机隐藏右侧快捷区，Enter 换行，点击箭头发送。实际问答需要自己的模型凭据；布局规范见[聊天风格 skill](../plugins/dsh-example/skills/dsh-chat-style/SKILL.md)。
