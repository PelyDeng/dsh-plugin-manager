...
2026-09-06，两位独立 agents 分别从首次使用者和已有站点维护者角度复审，未发现阻断问题。源码选择、构建及恢复相关的 25 项测试在公共仓库和私有集成仓库通过；覆盖使用不同本地宿主提交、缺少源码时不下载、缺少 pnpm 时准备工具链，以及重复执行保留配置。

Linux Docker 隔离验收完成首次完整源码构建和启动，宿主为仓库提供的 `0.1.2-alpha.5`，管理器为 `0.2.3`。未配置模型密钥时，auth 和 example 健康接口可用；管理员首次改密、example 授权访问及匿名访问拒绝均通过。首次启动使用提交 `305e68c`，随后使用 `0482ea9` 验证更新与恢复；后者移除了宿主下载和预设版本匹配检查。

更新启动期间对隔离容器发送 SIGKILL 后，部署记录保留失败状态、停服备份及残留 OWNER/LOCK。执行 `--resume` 后，同一操作的镜像和插件归档恢复启动，残留记录已备存，站点及插件设置摘要不变，修改后的管理员密码仍有效。隔离实例已停止，运行证据及备份保留在其 `.local/`。本次未执行浏览器交互、真实模型问答或生产升级。
