...
`/auth` 的密钥管理支持 DeepSeek 和智谱，`set-api-key` 命令只管理 DeepSeek。网页和脚本写入官方存储时，默认宿主通过自身凭据服务/文件监听生效，无需重启；自定义存储或关闭监听时使用当前运行服务的网页入口。文件模式不会把密钥导入 `.credentials.yaml`。由非空改为空并重启后，会重新使用官方已有来源，不表示删除旧密钥；若启动环境仍注入同名值，网页仍只读。

密钥不会创建模型路由、选择默认模型或验证额度。管理员在 `/auth` →“模型设置”的模型卡片中选择新会话默认模型；目录与选择由同一 DSH 实例的官方服务提供，保存无需重启。提供方路由及凭据引用仍由官方设置或 patch 管理。已有会话保留官方记录中的模型选择；真实问答须单独验收。
