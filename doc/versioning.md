...
公共框架当前版本为 **0.19.4**。根 `package.json` 的 `version` 是唯一版本源，manager、kit、auth、example 使用同一版本，作为一个单元构建、验证和发布。组件的 `package.json`、构建产物和文档中的当前版本都从根版本同步。
...
node scripts/version.mjs set 0.19.4
