...
- 高层 recover 不支持换修复包；同包配置修正不改变包、工具、镜像、选集或认证控制字段，并核对前序候选的安装归属。不得通过删锁、改状态或删数据绕过，健康通过不能代替登录授权和业务请求验证。
