...
- 外部产物是第一部署方式：作者独立项目用现有 pack 交付完整清单与归档，站点 incoming 是完整集合。精简部署包默认 archives，旧源码入口默认 source；漏包不表示停用，不自动挑版本，不构建作者源码。旧实例配置/数据路径不迁移。
...
- 换修复包的高层恢复延期；同包配置 recover 不改变包、工具、镜像、选集或认证控制字段。不得通过删锁、改状态或删数据绕过，健康通过不能代替登录授权和业务请求验证。
