...
| 角色 | 常用入口 | 不需要提前理解 |
| --- | --- | --- |
| 插件作者 | starter、`list / check / pack / verify-package` | 站点恢复、数据迁移和高级部署模式 |
| 部署者 | 部署包 build、health、实际业务请求 | 插件作者源码和构建工具链 |
| 站点维护者 | 源码发版、按需复用、resume / recover、迁移 | 每个业务插件的内部实现 |

这个边界也体现在命令行帮助中。外部作者产物是第一部署方式；源码构建和恢复规则保留给维护者。
