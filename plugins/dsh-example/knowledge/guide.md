...
适用：框架 0.14.0（manager、kit、auth、example 同一发布版本）；宿主以 gitlink 为准。本文是知识快照，不证明远程站点已升级。

## 如何统一新会话的默认模型？

Auth“模型设置”单选后保存至官方 `agentDefaultModel` / `settings`，仅影响新会话。插件接入 kit `conversationModel(ctx, id?, eventCount?)`，先核验所有权：无 ID 读默认，有 ID 恢复原模型，分支传继承事件数。读取失败拒绝恢复；硬编码模型不自动改变，专用识图可保留。详见 kit README。
