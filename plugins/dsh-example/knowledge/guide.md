...
适用：框架 0.14.1（manager、kit、auth、example 同一发布版本）；宿主以实际检出与交付验证记录为准。本文是知识快照，不证明远程站点已升级。
...
源码部署消费已有的独立 DeepSeek Harness 源码，记录实际提交；gitlink 只是检出参考，构建不自动更新宿主或要求匹配预设提交。在宿主目录按自己的 packageManager 和锁文件安装、构建。源码版本号不等于同名 npm 包已发布，不能把历史 SDK 依赖版本当作当前运行宿主。Node CLI 可通过部署配置的 harnessRoot 指向已准备的宿主源码目录；已安装的兼容宿主则使用 dshCliJs，二选一。默认密钥管理需要官方 credentials 服务和 credentials-local 存储；具体步骤见图文接入手册。
...
管理员在 `/auth`“模型设置”管理默认模型及 DeepSeek/智谱凭据；默认模型和官方凭据保存默认无需重启，非空 env 密钥覆盖对应凭据并使其只读。其他提供方按同一 DSH home 的官方设置配置。管理器打印官方认证地址文件位置（默认 `<交付根>/.local/data/dsh-web-auth-url.txt`），仅本机读取，不公开 token。独立交付的密钥命令见 manager DELIVERY.md；命令不切换模型。用新对话核实真实调用。
...
根 `package.json` 的 `version` 是唯一版本源，manager、kit、auth、example 同步发布；定制插件自行版本化，宿主版本独立记录，不随框架同步。JSON 保留实际版本值，不写变量。框架根运行 `node scripts/version.mjs set <版本>` 同步四个包及登记模板；模板用版本占位符（FRAMEWORK_VERSION，左右各两个花括号），`sync` 生成 Markdown，`check` 只校验。GitHub 不渲染 Markdown 变量。源码见 `scripts/version.mjs`，规则见 `doc/versioning.md`。

`.github/workflows/check.yml` 配置 Windows、macOS、Ubuntu × Node 22/24 共六组检查，验证公共工作区构建、类型、测试和独立归档消费，防止“只在作者仓库能用”。归档类型检查会验证公开声明需要的依赖；kit 子路径让仅用模型接口的作者不必安装工具类型依赖。另有 `.github/workflows/release.yml` 的 build/publish 两个发布任务；同一提交也触发发布流程时可显示八项，不代表多了两个操作系统。数量以该提交实际触发的工作流为准，检查通过不等于生产或真实模型验收。
...
| 问题 | 优先阅读 |
| --- | --- |
| 默认模型保存、恢复和分支 | `plugins/dsh-auth/src/models.ts`、`packages/plugin-kit/src/models.ts`、`plugins/dsh-example/src/index.ts` |
| 三平台入口、更新与锁 | `deploy/build.sh`、`deploy/build.ps1`、`deploy/scripts/release.mjs`、`deploy/scripts/build.mjs` |
| 会话预览与逐项归档 | `packages/plugin-kit/src/conversations.ts`、`doc/conversation-management.md` |
| 统一版本与自动检查 | `scripts/version.mjs`、`.github/workflows/check.yml`、`.github/workflows/release.yml` |
| 知识构建与检索边界 | `plugins/dsh-example/scripts/build-reference.mjs`、`plugins/dsh-example/src/framework.ts` |
