...
export function createConversationHistory({mount,toggle,newConversation,openConversation,currentId,list,mutate,read,onDeleted,label='历史对话',storageKey='chat-history'}){
  ...
  const mobile=matchMedia('(max-width: 960px)'),selected=new Set();let items=[],offset=null,query='',version=0,loading=false,multi=false,mutating=false,blocked=false,timer,disposed=false
  ...
  function confirmDelete(ids){if(!ids.length)return;const {d,body,actions}=dialog(`删除 ${ids.length} 条对话？`),cancel=element('button','取消'),remove=element('button','删除','danger'),error=element('p',undefined,'qh-error');body.append(element('p','将从历史列表移除，无法从此页面恢复。博客文章、附件文件和官方留存日志不会被删除。'),error);error.setAttribute('role','alert');actions.append(cancel,remove);cancel.onclick=()=>d.close();remove.onclick=async()=>{remove.disabled=true;if(await update({operation:'delete',ids}))d.close();else{error.textContent=status.textContent;remove.disabled=false}};d.showModal();cancel.focus()}
  ...
  layout();return{refresh,render,show,hide,setBusy(value){blocked=!!value;for(const b of rows.querySelectorAll('.qh-title')){b.disabled=blocked;b.title=blocked?'请等待回答完成或先停止':b.textContent}},get loading(){return loading},dispose(){disposed=true;version++;clearTimeout(timer);mobile.removeEventListener('change',layout);panel.remove();menu.remove()}}
}
