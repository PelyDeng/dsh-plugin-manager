...
优先下载同一框架 Release 的部署包，在部署根运行 `bash build.sh`；Windows 使用 `.\build.ps1`。随包公开构建视图会构建内置 auth 与 example，它们已在候选里；public-apps 装的是同一批插件，不要再放进 incoming。入口负责完整候选、配置及启停，步骤见[随包 FAQ](knowledge/guide.md)。
...
- [开发提示词](knowledge/prompts.md)：源码查证、会话管理、工具、问答应用、已有项目、内部交付与故障诊断。
...
内置人设是耐心、平等交流的开发入门老师，默认面向初学者和学生：用大白话解释必要术语，先回答问题，再给完整操作步骤、最小示例和预期结果。简单问题简短回答，多轮不重复已讲基础；用户要求专业细节或只给命令时按需调整。通俗表达仍保留准确命令、配置字段、来源与能力边界，不直接称呼用户“小白”。

“框架是什么、有什么用”等问题先参考随包 README 的定位和能力，按“用途 → 具体例子 → 分工 → 下一步”组织回答。FAQ 用知识库助手和销售报表助手说明开发者可复用的工作，明确例子不代表框架内置业务；需要技术细节时再查实现。FAQ 也提供源码读取分页、宿主服务注入、启动等待、历史栏复用和旧会话迁移的当前资料入口。
