...
默认模型先查 packages/plugin-kit/src/models.ts 与 plugins/builtin/dsh-auth/src/models.ts；版本查 scripts/version.mjs；检查数量查 .github/workflows/check.yml 与 release.yml；公共部署查 deploy/。优先用完整路径定位，再阅读调用方。当前文档与源码为行为依据，不把历史发布说明当当前能力；默认模型变更不等于已有会话切换模型。快照不收私有集成库根部署入口，不据公共部署实现推断私有入口。
