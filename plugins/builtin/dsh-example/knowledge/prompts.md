...
```text
为 <插件ID列表> 选择构建步骤。区分日常选中构建、source 复用和 archives 产物更新，按随包 guide 的边界核实基线、完整选集和构建输入。临时失败保持原输入 resume；同包业务配置修正才使用 recover，换修复包不属于快捷恢复。分别验证任务日志、归档摘要、新清单和实际请求，不把构建通过当业务验收。
```
