...
模型查 packages/plugin-kit/src/models.ts、plugins/builtin/dsh-auth/src/models.ts；版本查 scripts/version.mjs；CI查 .github/workflows；公共部署查 deploy/。不把快照当生产状态，不据公共代码推断私有入口，不用历史发布说明代替当前实现，不修改代码。
```

## 会话管理接入

```text
按 doc/conversation-management.md 和 example，为 <插件ID> 接入本人查询、只读预览和逐项官方归档。复用 owner 索引与删除流程，验证撤权、旧数据、并发、部分失败重试与独立安装包验证，不操作生产数据。
```
...
```text
评估 <作者包根> 的 <项目> 接入框架 <版本>、宿主 <版本>。比较外部服务加适配插件与现有 Node 入口，选择最小方案。采用 --root <作者包根> --package . 的 pnpm 单包流程；不承诺多包自动发现或外部 development/link。补声明、锁文件、配置校验和 README，验证离开作者目录后的安装和运行。
```
...
```text
为 <插件ID列表> 规划构建步骤。区分日常选中构建（`pnpm build/package --plugins`）与站点 build：站点内置插件固定全量构建、不按旧成功记录复用，archives 站点用随包公开源码材料重建内置插件，外部产物只来自 incoming。按随包 guide 的边界核实基线、完整选集和构建输入。失败或中断后修正输入直接重跑普通 build（内置插件固定全量构建，没有按插件点名重建的开关），换修复包要整体替换 incoming 对应目录。分别验证任务日志、归档摘要、新清单和实际请求，不把构建通过当业务验收。
```
