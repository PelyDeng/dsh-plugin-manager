...
这是框架 0.19.3 随包指南。模型不可用时仍可在已授权应用中阅读；内容来自发布时固定的公共文档，不扫描部署者机器。在线 main 可能领先，知识不能证明生产状态。
...
需要 Node.js `^22.19.0 || >=24`、pnpm `11.19.0` 和系统 tar。从同一框架 Release 取得 `dsh-plugin-manager-starters-0.19.3.zip`、`plugin-manager-0.19.3.tgz`；起步包的鉴权目录已带同版 kit；仅单独复制仓库示例或升级 kit 时另取 `plugin-kit-0.19.3.tgz`。核对随发行提供的 SHA-256，不假设这些包已发布到 npm registry。
...
```sh
pnpm init
pnpm add --ignore-workspace /absolute/path/plugin-manager-0.19.3.tgz
...
从同一个框架 Release 取得 `dsh-plugin-manager-deployment-0.19.3.zip` 并解压。准备 Node.js `^22.19.0 || >=24`、系统 tar、本机 Linux Docker 引擎及 Compose；不自动安装系统软件。首次 build 会在随包公开构建视图（`source/` 与 `tools/builtin-build/`）里安装框架工作区依赖，并按站点根清单准备固定版本的 pnpm；这一步需要网络或完整缓存，站点自身目录不会安装依赖。镜像架构必须有该版本实际提供的运行镜像，不使用未验证的默认摘要。
...
- [固定版本作者指南](https://github.com/PelyDeng/dsh-plugin-manager/blob/v0.19.3/doc/plugin-development.md)
- [固定版本部署指南](https://github.com/PelyDeng/dsh-plugin-manager/blob/v0.19.3/doc/first-deployment.md)
- [可复制开发提示词](prompts.md)
