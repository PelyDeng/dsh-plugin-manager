...
test('deployment questions reach the Agent with builtin build and retry boundaries', async () => {
  const f = await fixture({ mode: 'standalone' })
  let response
  try {
    response = await f.request('/chat', { message: '只改 C 能只构建 C 吗？站点构建和重试怎么进行？' })
    const knowledge = f.handles[0].sections.find(section => section.name === 'example:knowledge').text
    for (const fact of ['pnpm exec dsh-plugin-manager pack', '内置插件固定全量构建', 'migrate-site', '缺外部归档时在停旧前报错', '从当前现场重新收敛', '保留授权与安装层']) expect(knowledge).toContain(fact)
    const faq = await f.request('/guide.md')
    expect(faq.status).toBe(200)
    expect(await faq.text()).toContain('内置插件')
  } finally { await response?.body.cancel(); await f.close() }
})
...
test('current model, version and CI questions have source evidence in the shipped Agent tools', async () => {
  ...
  try {
    ...
    for (const [path, expected] of [
      ...
      ['plugins/builtin/dsh-auth/src/models.ts', /saveSelection/],
      ...
      ['plugins/builtin/dsh-example/web/conversation-history.js', /dialogs\.clear/],
    ]) {
