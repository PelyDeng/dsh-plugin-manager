...
test('authorized conversations can cite actual source, but cannot read runtime files or act as another Agent', async () => {
  ...
  try {
    ...
    const found = JSON.parse(await search.execute({ query: 'synchronize managed 授权集合' }, execution))
    expect(found.results.some(result => result.path === 'packages/plugin-manager/src/installation.mjs')).toBe(true)
    // 行号随实现变化：从源码定位 synchronize 的起始行，再核对读取窗口的内容与分页。
    const framework = readFileSync(new URL('../../../../packages/plugin-manager/src/installation.mjs', import.meta.url), 'utf8').split('\n')
    const start = framework.findIndex(line => line.includes('export async function synchronize')) + 1
    expect(start).toBeGreaterThan(0)
    const source = JSON.parse(await read.execute({ path: 'packages/plugin-manager/src/installation.mjs', startLine: start, lines: 60 }, execution))
    expect(source.content).toContain('managed')
    expect(source.content).toContain('assessVerification')
    expect(source.nextLine).toBe(start + 60)
...
test('source snapshot includes all public framework layers and excludes private plugins and runtime configuration', () => {
  ...
  try {
    ...
    for (const path of ['packages/plugin-manager/src', 'packages/plugin-kit/src', 'plugins/builtin/dsh-auth/src', 'plugins/builtin/dsh-example/src', 'plugins/custom-private-app/src', 'deploy', '.local']) {
      mkdirSync(join(root, path), { recursive: true }); writeFileSync(join(root, path, 'implementation.mjs'), `// ${path}`)
    }
    writeFileSync(join(root, 'plugins/builtin/dsh-example', 'config.json'), '{"credential":"private-fixture"}')
    ...
    expect(value.files.some(file => file.path === 'plugins/builtin/dsh-auth/src/implementation.mjs')).toBe(true)
