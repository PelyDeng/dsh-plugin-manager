...
LABEL com.dsh-plugin.manager.sha256=${MANAGER_SHA256} \
      com.dsh-plugin.framework.revision=${FRAMEWORK_REVISION}
...
RUN test -n "$MANAGER_SHA256" \
    ...
    && node /opt/plugin-manager/node_modules/@dsh-plugin/plugin-manager/dist/cli.mjs --help
USER node
