...
默认只显示正在执行的步骤、完成或失败结果，以及最终访问地址和发布记录。交互终端使用动态点提示等待；重定向输出或 CI 环境使用普通步骤行。pnpm、打包器和 Docker 的详细输出保存在 `.local/artifacts/build-logs/build-*.log`，每次运行打印其路径。失败时显示最后 12 行诊断信息，完整日志仍保留；Linux 日志文件仅当前用户可读写。构建与校验照常执行。
