...
默认只显示正在执行的步骤、完成或失败结果，以及最终访问地址和发布记录。进度百分比按环境检查、发布物准备、镜像准备、备份、部署验证五个阶段等分计算，不代表已消耗时间；首次部署无需备份，恢复部署可复用已验证的发布物和镜像，这些阶段确认可跳过后计为完成。交互终端的进度条随百分比填充，执行中的光标闪动，但不会随等待时间虚增百分比；全部成功退出后才显示 100%。重定向输出或 CI 环境使用带百分比的普通步骤行。pnpm、打包器和 Docker 的详细输出保存在 `.local/artifacts/build-logs/build-*.log`，每次运行打印其路径。失败时保留当前百分比并显示最后 12 行诊断信息，完整日志仍保留；Linux 日志文件仅当前用户可读写。构建与校验照常执行。
