...
Docker 实例通过 `apply-compose --root <项目根> --config <deployment.json>` 应用配置，接受不可变本机镜像 ID 或 registry 摘要，`--resume` 恢复原操作。可先运行 `check-compose` 生成候选文件并检查挂载权限，不停止服务或启动 DSH；`apply-compose` 也会先执行这些检查。仅支持本机 unix/npipe endpoint 的 Linux 引擎；Windows/macOS 与 Linux Docker Desktop 使用 bridge 和宿主回环端口映射，原生 Linux 保留 host 网络。macOS 尚未完成真机验收。
