...
`release-site --root <站点> [--config <配置>] [--resume | --recover --data-compatible]` 是部署包 build 使用的统一编排入口；普通用户仍运行 build。recover 仅修正同包业务配置，不支持借此替换错误包。源码适配、锁和恢复说明见对应版本的[运维文档](https://github.com/PelyDeng/dsh-plugin-manager/blob/v0.17.0/deploy/README.md)。
