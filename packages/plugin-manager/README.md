...
用于打包、安装和管理 DSH 应用的命令行工具（CLI）。独立包和内部 `plugins/builtin/*` 共用 build → check → pack，通过官方 DSH CLI 安装 Bundle、配置并受控启停。kit 和基础认证可选；管理声明不会自动保护业务路由。

## 安装与作者操作

<!-- Excerpt from doc/plugin-development.md.tmpl#author-tools; edit its source. -->
需要 Node.js `^22.19.0 || >=24`、pnpm `11.19.0` 和系统 tar。从同一框架 Release 取得 `dsh-plugin-manager-starters-0.19.5.zip`、`plugin-manager-0.19.5.tgz`；起步包的鉴权目录已带同版 kit；仅单独复制仓库示例或升级 kit 时另取 `plugin-kit-0.19.5.tgz`。核对随发行提供的 SHA-256，不假设这些包已发布到 npm registry。
...
```sh
pnpm init
pnpm add --ignore-workspace /absolute/path/plugin-manager-0.19.5.tgz
...
`release-site --root <站点> [--config <配置>]` 是部署包 build 使用的统一编排入口；普通用户仍运行 build。内置插件固定全量构建，外部产物来自 incoming；失败后直接重新运行普通 build。旧站点升级用 `migrate-site` 一次性迁移。源码适配、锁和重试说明见对应版本的[运维文档](https://github.com/PelyDeng/dsh-plugin-manager/blob/v0.19.5/deploy/README.md)。

## 内部批量开发与 API

省略 `--package` 时保留根锁文件及 `plugins/builtin/*` 扫描，支持 `--plugins "auth,example"`、默认选集、all/none。逗号分隔的 ID 列表加引号，避免 PowerShell 拆成数组。内部 kit 工作区准备和批量操作保持原行为。
