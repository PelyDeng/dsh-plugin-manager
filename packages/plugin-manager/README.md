...
`release-site --root <站点> [--config <配置>]` 是部署包 build 使用的统一编排入口；普通用户仍运行 build。内置插件固定全量构建，外部产物来自 incoming；失败后直接重新运行普通 build。旧站点升级用 `migrate-site` 一次性迁移。源码适配、锁和重试说明见对应版本的[运维文档](https://github.com/PelyDeng/dsh-plugin-manager/blob/v0.19.4/deploy/README.md)。
