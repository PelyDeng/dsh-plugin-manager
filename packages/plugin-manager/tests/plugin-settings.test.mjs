...
test('apply generates one complete Compose document and preserves existing settings', t => {
  ...
  // 两次 up 都等待健康：切换 restart 会重建容器，最终实例必须重新验证过才算 ready。
  assert.deepEqual(calls.map(args => args.slice(5)), [['stop', 'dsh'], ['up', '-d', '--force-recreate', '--wait', '--wait-timeout', '180', 'dsh'], ['up', '-d', '--wait', '--wait-timeout', '180', 'dsh']]);
