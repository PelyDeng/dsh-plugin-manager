...
test('the package pipeline builds and packs a portable manifest, and the standalone check verifies that build', async t => {
  ...
  assert.match(spawnSync(tarCommand, ['-xOf', resolve(root, 'updated-output', updated.plugins[0].archive), 'package/README.md'], { encoding: 'utf8' }).stdout, /Updated bytes/);
  ...
  const bad = spawnSync(tarCommand, ['-czf', resolve(root, 'private.tgz'), '-C', resolve(root, 'leak'), 'package/env.conf'], { encoding: 'utf8' });
