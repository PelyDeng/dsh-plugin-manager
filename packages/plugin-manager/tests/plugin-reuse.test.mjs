...
function fixture(t, dependencies = {}) {
  ...
  const site = { plugins: ['a', 'b', 'c', 'd'], artifacts: '.local/runtime', composeProject: 'fixture' };
  function publish(rebuilt = site.plugins, reuse) {
    ...
    save(recordPath, { schemaVersion: 2, operation, revision, hostCommit, hostSourceClean: true, hostSourceCommit: hostCommit, status: 'ready', manifest, manifestHash: digest(manifest), candidatePath, candidateHash: digest(candidatePath), image, buildEnvironment: environment, pluginBuilds });
  ...
  return { root, host, site, git, hostCommit, publish, get prior() { return prior; }, get revision() { return revision; },
    ...
    prepare(rebuilt = ['c'], overrides = {}) { return preparePluginReuse({ root, ...prior, site, revision, hostCommit, buildEnvironment: environment, rebuilt, git, ...overrides }); },
