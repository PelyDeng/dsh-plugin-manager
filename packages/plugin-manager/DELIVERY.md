...
适用于 manager 0.16.2。部署者只需管理工具、匹配的官方 DSH、应用与认证插件各自的发布目录，以及作者提供的配置模板；无需作者源码。以下 `dsh-plugin-manager` 指已安装 CLI，本地工具目录安装时用 `pnpm exec dsh-plugin-manager`。需要 Node.js `^22.19.0 || >=24`、pnpm `11.19.0` 和系统 tar。
...
```sh
pnpm add --ignore-workspace /path/to/plugin-manager-0.16.2.tgz
pnpm exec dsh-plugin-manager --version
```

宿主应使用应用交付说明所要求的版本及依赖，内置 auth/example 的要求见对应版本的[宿主兼容说明](https://github.com/PelyDeng/dsh-plugin-manager/blob/v0.16.2/doc/host-compatibility.md)。管理器不自动下载、更新或构建宿主。运行配置二选一：
...
独立 CLI 与站点 build 的 source/archives 不混用。完整字段参考对应版本的[框架配置](https://github.com/PelyDeng/dsh-plugin-manager/blob/v0.16.2/doc/framework-configuration.md)。本文件保留完整手工交付步骤，可脱离仓库阅读。
