 ...
 */
export const pluginCategoryLabels = {
  'system-default': '系统默认',
...
export type PluginCategory = keyof typeof pluginCategoryLabels

/** 已知分类的展示顺序；未列出的分类排在它们之后。 */
export const pluginCategoryOrder: readonly PluginCategory[] = ['system-default', 'universal-tools', 'agents', 'web-services']
...
/** Cordis 事件通道的类型增强只有一处声明，见 events.ts 的说明。 */
...
export function listPlugins(ctx: Context): PluginDescriptor[] {
  const entries: (import('./events.ts').CatalogEntry)[] = []
