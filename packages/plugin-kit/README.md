...
| 导出 | 用途 |
| --- | --- |
| `@dsh-plugin/plugin-kit/access` | `createAccess`、身份、认证提供者、运行目录和撤权通知 |
| `@dsh-plugin/plugin-kit/http` | `createPluginHttp`：受保护路由与显式公开探针 |
| `@dsh-plugin/plugin-kit/tools` | `createPluginTools`、`guardTool`：执行前后鉴权与工具登记 |
| `@dsh-plugin/plugin-kit/route-path` | 无宿主依赖的规范路由校验 |
| `@dsh-plugin/plugin-kit` | 上述访问、HTTP 和工具 API 的统一导出 |
