...
公开文档从[接入手册](doc/getting-started.md)导航到作者、部署、配置参考。能力变化需同步 example 随包 knowledge 的 FAQ/提示词；它们就是助手读取的知识，不另维护一份答案。校验来源、命令、知识大小及实际模型输入；真实模型问答与替身测试分别说明。`doc/` 仅保存当前产品的使用、开发、配置、部署、故障排查和版本资料。个人博客、推广与邀请材料、开发计划、评审报告、验收记录和临时实验放在仓库外；不要把待办或本次执行结果写入使用手册。必要的兼容条件、权限规则与数据保护要求写在对应操作处。维护者发布流程见 [.github/RELEASING.md](.github/RELEASING.md)。
