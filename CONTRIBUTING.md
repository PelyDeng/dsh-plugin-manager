...
公开文档从[接入手册](doc/getting-started.md)导航到作者、部署、配置参考。能力变化需同步 example 随包 knowledge 的 FAQ/提示词；它们就是助手读取的知识，不另维护一份答案。校验来源、命令、知识大小及实际模型输入；真实模型问答与替身测试分别说明。设计草案、评审讨论和临时测试过程放在忽略的本地目录，不作为面向使用者的说明发布。
