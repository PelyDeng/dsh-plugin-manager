...
Release 起步包已带同版 vendor/plugin-kit.tgz 和相对开发依赖，在本作者目录执行 `pnpm install --ignore-workspace`。仅单独复制仓库示例时，从 Release 取得 `plugin-kit-0.18.0.tgz` 并保存为 vendor/plugin-kit.tgz，再执行：
