...
复制到自己的仓库，取得维护者提供的 plugin-kit 0.15.1 tgz 与 plugin-manager 0.15.1。该目录故意不引用未发布的 registry 包；先把实际 kit 归档加入开发依赖，生成自己的锁文件：
...
默认要求认证。通过管理器部署时，候选清单必须同时包含一个认证提供者（auth provider）和本应用（consumer），并配置 publicOrigin。已经安装在 profile 中但未出现在候选清单的 auth 不满足此要求。通过 `dsh-plugin-manager compose-release --root <交付根> --output <新目录> --manifest <auth清单> --manifest <本应用清单>` 生成包含全部所需插件的候选清单，再按 manager 包内 DELIVERY.md 部署。
